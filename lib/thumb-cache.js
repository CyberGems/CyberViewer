...
/**
 * Remove generated thumbnail files without touching any other user data.
 * @returns {{ removed: number, freedBytes: number }}
 */
function clearThumbCache(cacheDir) {
  if (!fs.existsSync(cacheDir)) {
    return { removed: 0, freedBytes: 0 };
  }
  ...
  let names;
  try {
    names = fs.readdirSync(cacheDir).filter((name) => name.endsWith('.jpg'));
  } catch (_) {
    return { removed: 0, freedBytes: 0 };
  }
  ...
  for (const name of names) {
    ...
    try {
      ...
      // Skip files that are locked or disappeared during cleanup.
    }
  }
...
module.exports = { clearThumbCache, evictThumbCache, DEFAULT_MAX_BYTES, DEFAULT_MAX_FILES };
